...
def compute_residue_contact_maps_fixed(
    ...
    """
    Compatibility API used by other analysis modules.

    Returns the original chain-resolved tensors:
      ref_sel_inter: (n_ref, n_sel, L_ref, L_sel)
      ref_ref_inter: (n_ref, n_ref, L_ref, L_ref)
      sel_sel_inter: (n_sel, n_sel, L_sel, L_sel)
    ...
    rca = _safe_lookup(ref_chain, a)
    rcb = _safe_lookup(ref_chain, b)
    rpa = _safe_lookup(ref_pos, a)
    rpb = _safe_lookup(ref_pos, b)

    if selection_chains is not None:
        sca = _safe_lookup(sel_chain, a)
        scb = _safe_lookup(sel_chain, b)
        spa = _safe_lookup(sel_pos, a)
        spb = _safe_lookup(sel_pos, b)
...
def accumulate_residue_contact_maps_fixed(
    pairs,
    reference_chains,  # n_ref x L_ref (lists)
    selection_chains,  # n_sel x L_sel (lists) or None
    reference_lookup,
    selection_lookup=None,
    undirected=True,
    unique_pairs=True,
    drop_self_pairs=True,
    dtype=np.int64
    ...
    """
    Aggregate residue-residue contacts directly into the 2D matrices that are
    ultimately written or reduced into time-series values.
    """
    n_ref = len(reference_chains)
    L_ref = len(reference_chains[0]) if n_ref else 0
    if n_ref and any(len(ch) != L_ref for ch in reference_chains):
    ...
    ref_ref_inter = np.zeros((L_ref, L_ref), dtype=dtype)
    ref_intra = np.zeros((L_ref, L_ref), dtype=dtype)

    if selection_chains is not None:
        ref_sel_inter = np.zeros((L_ref, L_sel), dtype=dtype)
        sel_sel_inter = np.zeros((L_sel, L_sel), dtype=dtype)
        sel_intra = np.zeros((L_sel, L_sel), dtype=dtype)
    ...
    ref_chain, ref_pos = reference_lookup
    rca = _safe_lookup(ref_chain, a); rcb = _safe_lookup(ref_chain, b)
    rpa = _safe_lookup(ref_pos, a); rpb = _safe_lookup(ref_pos, b)

    if selection_chains is not None:
        sel_chain, sel_pos = selection_lookup
        sca = _safe_lookup(sel_chain, a); scb = _safe_lookup(sel_chain, b)
        spa = _safe_lookup(sel_pos, a); spb = _safe_lookup(sel_pos, b)

    m = (rca >= 0) & (rcb >= 0) & (rca == rcb)
    if np.any(m):
        np.add.at(ref_intra, (rpa[m], rpb[m]), 1)
        if undirected:
            np.add.at(ref_intra, (rpb[m], rpa[m]), 1)

    m = (rca >= 0) & (rcb >= 0) & (rca != rcb)
    if np.any(m):
        np.add.at(ref_ref_inter, (rpa[m], rpb[m]), 1)
        if undirected:
            np.add.at(ref_ref_inter, (rpb[m], rpa[m]), 1)

    if selection_chains is not None:

        m = (sca >= 0) & (scb >= 0) & (sca == scb)
        if np.any(m):
            np.add.at(sel_intra, (spa[m], spb[m]), 1)
            if undirected:
                np.add.at(sel_intra, (spb[m], spa[m]), 1)

        m = (sca >= 0) & (scb >= 0) & (sca != scb)
        if np.any(m):
            np.add.at(sel_sel_inter, (spa[m], spb[m]), 1)
            if undirected:
                np.add.at(sel_sel_inter, (spb[m], spa[m]), 1)

        m = (rca >= 0) & (scb >= 0)
        if np.any(m):
            np.add.at(ref_sel_inter, (rpa[m], spb[m]), 1)

        m = (sca >= 0) & (rcb >= 0)
        if np.any(m):
            np.add.at(ref_sel_inter, (rpb[m], spa[m]), 1)
...
def contact(args):
    ...
    if has_selection:
        search_atom_indices = np.union1d(reference_group.indices, selection_group.indices).astype(np.int64, copy=False)
    ...
    max_lookup_idx = int(search_atom_indices.max()) if search_atom_indices.size else 0
    reference_lookup = _build_lookup_fixed(reference_chains, max_lookup_idx)
    selection_lookup = _build_lookup_fixed(selection_chains, max_lookup_idx) if has_selection else None
    ...
    for ts in tqdm(trajectory.Universe.trajectory[start_frame:end_frame:interval_frame]):
        ...
        maps = accumulate_residue_contact_maps_fixed(
            pairs_atomid,
            reference_chains,
        ...
        ref_intra = maps["ref_intra"]
        ref_ref = maps["ref_ref_inter"]
        mean_ref_intra = ref_intra / len(reference_chains) if args.intra_average else ref_intra
        mean_ref_intra = remove_near_diagonal(mean_ref_intra, args.remove_diagonal)
        mean_ref_ref_inter = ref_ref

        if has_selection:

            sel_intra = maps["sel_intra"]
            ref_sel = maps["ref_sel_inter"]
            sel_sel = maps["sel_sel_inter"]
            mean_sel_intra = sel_intra / len(selection_chains) if args.intra_average else sel_intra
            mean_sel_intra = remove_near_diagonal(mean_sel_intra, args.remove_diagonal)
            mean_ref_sel = ref_sel
            mean_sel_sel_inter = sel_sel
